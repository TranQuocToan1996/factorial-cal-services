...
func (h *FactorialMessageHandler) Handle(ctx context.Context, body []byte) error {
	...
	if err != nil {
		...
		h.repository.UpdateStatus(message.Number, "failed")
	...
	// Update DB with S3 key and status=Done
	if err := h.repository.UpdateS3Key(message.Number, s3Key, domain.StatusDone); err != nil {
